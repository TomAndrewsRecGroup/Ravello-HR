...
export default function PolicyHealthcheckTool() {
  ...
  const present   = policies.length - missing.length;
  const percentage= Math.round((present / policies.length) * 100);

  if (stage === 'gate') {
    return (
      <EnquiryGate
        toolName="Policy Healthcheck"
        source="policy_healthcheck"
        teaserScore={`${percentage}%`}
        teaserLabel="policy coverage"
        result={{
          percentage,
          present,
          total:    policies.length,
          gaps:     missing.map((p) => `${p.label} (${p.priority})`),
          critical: critical.map((p) => p.label),
        }}
    ...
    return (
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
        ...
        <div className="bg-[var(--brand-navy)] rounded-xl p-6 text-center mt-6">
          <p className="text-white font-semibold mb-2">PolicySafe&trade; fixes all of this.</p>
          <p className="text-white/70 text-sm mb-4">Our Gold package covers every document on this list, tailored to your business, ready in five working days.</p>
          <a href="/policysafe" className="btn-gradient">Explore PolicySafe&trade; <ArrowRight size={16} /></a>
        </div>
      </div>
    );
  }
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
      ...
      <div className="border-t border-gray-100 pt-6">
        ...
        <button onClick={() => setStage('gate')} className="btn-primary w-full justify-center">
          Generate My Gap Report <ArrowRight size={16} />
        </button>
      </div>
    </div>
  );
