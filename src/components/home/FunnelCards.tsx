...
const funnels = [
  ...
  },
  {
    ...
    accentBorder: 'rgba(91,155,255,0.25)',
    iconBg: 'rgba(91,155,255,0.1)',
    iconColor: '#5B9BFF',
  },
  {
    icon: Briefcase,
    ...
    accentBorder: 'rgba(13,21,53,0.18)',
    iconBg: 'rgba(13,21,53,0.07)',
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          {funnels.map((f) => {
            ...
            return (
              <div
                key={f.headline}
                className="relative flex flex-col rounded-[22px] p-8 bg-white group transition-all duration-300"
                style={{ border: '1px solid var(--brand-line)', boxShadow: '0 2px 6px rgba(13,21,53,0.04), 0 8px 32px rgba(13,21,53,0.05)' }}
                onMouseOver={(e) => {
                  const el = e.currentTarget as HTMLElement;
                  el.style.boxShadow = '0 6px 20px rgba(13,21,53,0.08), 0 24px 64px rgba(13,21,53,0.09)';
                  el.style.transform = 'translateY(-5px)';
                  el.style.borderColor = f.accentBorder;
                }}
                onMouseOut={(e) => {
                  const el = e.currentTarget as HTMLElement;
                  el.style.boxShadow = '0 2px 6px rgba(13,21,53,0.04), 0 8px 32px rgba(13,21,53,0.05)';
                  el.style.transform = 'translateY(0)';
                  el.style.borderColor = 'var(--brand-line)';
                }}
              >
                {/* Top colour bar */}
                <div className="absolute top-0 left-8 right-8 h-[3px] rounded-b-full" style={{ background: f.accent }} />

                {/* Icon */}
                <div className="w-12 h-12 rounded-[14px] flex items-center justify-center mb-6" style={{ background: f.iconBg }}>
                  <Icon size={22} style={{ color: f.iconColor }} />
                </div>

                <span className={f.pillStyle + ' mb-4 self-start'}>{f.pill}</span>

                <h3 className="font-bold text-[1.3rem] mb-3 leading-snug" style={{ color: 'var(--ink)', letterSpacing: '-0.015em' }}>{f.headline}</h3>
                <p className="text-sm leading-relaxed mb-6" style={{ color: 'var(--ink-soft)' }}>{f.body}</p>

                <ul className="space-y-3 mb-8 flex-1">
                  {f.pains.map((p) => (
                    <li key={p} className="flex items-start gap-3 text-sm" style={{ color: 'var(--ink-faint)' }}>
                      <span className="mt-[7px] w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ background: f.accent }} />
                      {p}
                    </li>
                  ))}
                </ul>
                ...
                <div className="mt-auto space-y-2.5">
                  ...
                  <Link href={f.link.href} className="flex items-center justify-center gap-1.5 text-sm font-medium py-2 rounded-[10px] transition-colors hover:bg-[var(--surface-soft)]" style={{ color: 'var(--ink-soft)' }}>
