      ...
      >
                ...
                return (
                  <li key={p.id} className="px-3">
                    <button onClick={() => toggleExpand(p.id)} className="w-full flex items-center gap-3 py-3 text-left">
                      ...
                      <span className="font-semibold text-sm" style={{ color: 'var(--ink)' }}>{p.company_name}</span>
                      {p.locations && (<span className="text-xs" style={{ color: 'var(--ink-soft)' }}>· {p.locations}</span>)}
                      {p.industry && (<span className="badge ml-auto" style={{ background: 'var(--surface-alt)', color: 'var(--ink-soft)' }}>{p.industry}</span>)}
                      {interestedHere > 0 && (
                        <span className="text-[11px] font-semibold" style={{ color: 'var(--purple)' }}>{interestedHere} matched</span>
                      )}
                    </button>
                    {isOpen && (
                      <div className="pb-3 pl-7 space-y-1.5">
                        {p.role_opportunities.length === 0 && (
                          <p className="text-xs italic" style={{ color: 'var(--ink-faint)' }}>No roles listed yet.</p>
                        )}
                        {p.role_opportunities.map(role => {
                          ...
                          return (
                            <label key={role.id}
                              ...
                              <span className="text-sm flex-1" style={{ color: 'var(--ink)' }}>{role.title}</span>
                              {role.location && (<span className="text-[11px]" style={{ color: 'var(--ink-faint)' }}>{role.location}</span>)}
                              {saved && (<Check size={13} style={{ color: 'var(--purple)' }} />)}
                            </label>
                          );
                        })}
                        {(() => {
            ...
            <ul className="space-y-1.5 max-h-[180px] overflow-y-auto">
              {interests.map(i => (
                <li key={i.id} className="flex items-center gap-2 text-xs">
                  ...
                  <select value={i.status} onChange={e => changeStatus(i.id, e.target.value as InterestStatus)}
                          className="input" style={{ padding: '3px 22px 3px 8px', fontSize: 11, width: 'auto' }}>
                    {(Object.keys(STATUS_LABEL) as InterestStatus[]).map(s => (<option key={s} value={s}>{STATUS_LABEL[s]}</option>))}
                  </select>
        ...
        <div className="px-6 py-4 flex items-center justify-between gap-3" style={{ borderTop: '1px solid var(--line)' }}>
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={save} disabled={saving || pending.size === 0} className="btn-cta btn-sm flex items-center gap-1.5">
