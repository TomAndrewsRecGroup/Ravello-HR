...
export default async function DevPlanPreviewPage({ params }: { params: { id: string } }) {
  ...
  return (
    <main
      className="print-area print-keep-color flex-1"
      style={{
    ...
    >
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-4 no-print">
          <Link href={`/dev-plans/${plan.id}`} className="inline-flex items-center gap-1 text-sm hover:underline" style={{ color: primary }}>
        ...
        >
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <h1 className="font-display text-3xl font-bold leading-tight" style={{ fontFamily }}>{plan.title}</h1>
              <p className="text-sm opacity-90 mt-2 font-semibold">
                {athleteName ? `For ${athleteName}` : ''}
                {athleteName && companyName ? ' • ' : ''}
                {companyName}
              </p>
        ...
        {ms.length > 0 && <SectionHeader icon={<Sparkles size={16} />} label="Roadmap" colour={primary} fontFamily={fontFamily} />}
        {ms.length === 0 ? null : (
                  ...
                  <div className="flex gap-4">
                    <div className="relative flex-shrink-0 flex flex-col items-center">
                      <div
                        ...
                        style={{ background: isDone ? secondary : primary, color: '#fff', fontFamily }}
                      >
                        {idx + 1}
                      </div>
                      {!isLast && (
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        <h3 className="font-semibold text-base" style={{ color: 'var(--ink)', fontFamily }}>{m.title}</h3>
                        <span
                      ...
                      {m.description && (
                        <p className="text-sm mt-2 leading-relaxed" style={{ color: 'var(--ink-soft)' }}>{m.description}</p>
                      )}
                      {m.due_date && (
...
function SectionHeader({ icon, label, colour, fontFamily }: { icon: React.ReactNode; label: string; colour: string; fontFamily?: string }) {
  return (
    <div className="flex items-center gap-2 mb-4 mt-2">
      <span className="w-9 h-9 rounded-lg flex items-center justify-center print-keep-color" style={{ background: hex(colour, 0.12), color: colour }}>
        {icon}
      </span>
    ...
    <section className="mb-8">
      ...
      <div className="grid sm:grid-cols-2 gap-3">
        {list.map((it, i) => (
          ...
          >
            {it.box1 && <h3 className="font-semibold mb-1" style={{ color: primary, fontFamily }}>{it.box1}</h3>}
            {it.box2 && (
              <p className="text-sm leading-relaxed" style={{ color: 'var(--ink-soft)', whiteSpace: 'pre-wrap' }}>{it.box2}</p>
            )}
            {accent && it.box1 && (
