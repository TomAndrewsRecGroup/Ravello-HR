...
import type { IvyLensTicket } from '@/lib/support/tickets';
...
interface Props {
  initialTickets: IvyLensTicket[];
}

export default function IvyLensSupportClient({ initialTickets }: Props) {
  const [tab, setTab] = useState<'create' | 'list'>('list');
  const [tickets, setTickets] = useState<IvyLensTicket[]>(initialTickets);
  ...
  // Refresh on window focus instead of polling — initial data is server-rendered.
  useEffect(() => {
