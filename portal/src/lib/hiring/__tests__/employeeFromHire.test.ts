...
const sql = readFileSync(resolve(__dirname, '../../../../../supabase/migrations/015_employee_records_leave_calendar.sql'), 'utf8');
...
const columns = new Set([...table.matchAll(/^\s+(\w+)\s+(UUID|TEXT|DATE|NUMERIC|INTEGER|BOOLEAN|TIMESTAMPTZ|employment_status)\b/gm)].map(m => m[1]));
