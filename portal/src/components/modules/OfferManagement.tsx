...
const STATUS_CONFIG: Record<string, { label: string; bg: string; color: string; icon: React.ElementType }> = {
  draft:            { label: 'Draft',            bg: 'rgba(148,163,184,0.12)', color: '#475569', icon: FileText },
  sent:             { label: 'Sent',             bg: 'rgba(59,111,255,0.12)',  color: '#1848CC', icon: Clock },
  verbal_accepted:  { label: 'Verbal Accepted',  bg: 'rgba(245,158,11,0.12)', color: '#92400E', icon: CheckCircle2 },
  written_accepted: { label: 'Written Accepted', bg: 'rgba(22,163,74,0.12)',  color: '#166534', icon: CheckCircle2 },
  declined:         { label: 'Declined',         bg: 'rgba(220,38,38,0.10)',  color: '#991B1B', icon: X },
  withdrawn:        { label: 'Withdrawn',        bg: 'rgba(220,38,38,0.10)',  color: '#991B1B', icon: X },
  lapsed:           { label: 'Lapsed',           bg: 'rgba(148,163,184,0.12)', color: '#475569', icon: AlertTriangle },
};
...
function OfferCard({ offer, onStatusChange }: { offer: Offer; onStatusChange: (id: string, status: string) => void }) {
  ...
  async function updateStatus(newStatus: string) {
    ...
    if (newStatus === 'withdrawn')        extra.withdrawn_at = now;
...
export default function OfferManagement({ requisitionId, companyId, candidates, initialOffers }: Props) {
  const supabase = createClient();
