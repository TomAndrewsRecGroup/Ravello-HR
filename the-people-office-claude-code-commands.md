# The People System: Claude Code Sprint Commands
## Ravello-HR Repo → The People System
...
# The People System: Sprint Context
...
2. Create a file called AUDIT.md in the repo root documenting:
   ...
   - What needs to change for The People System (content only, no layout/CSS changes)
   - Any pages that need to be created new
   - Any pages that should be removed or repurposed

3. Rewrite the homepage (src/app/page.tsx or equivalent) with The People System content:
...
4. Commit: `content: homepage rewritten for The People System`
```
...
Commit: `content: three conversion funnels rewritten for The People System`
```
...
Commit: `content: services pages and about page written for The People System`
```
...
4. Update meta titles, descriptions, and og tags on every page to reflect The People System brand and positioning.
...
1. Build Action Centre: add to dashboard as a full section and as its own page at /actions:
   ...
   - No active services on account → "Confirm your service package with The People System"
...
3. First-login onboarding flow:
   ...
   - Step 4: Book your onboarding call with The People System
...
Configure it to also write to The People System Supabase instance's bd_scanned_roles and bd_companies tables.
...
1. Update popup-bd.js (BD scanning mode):
   ...
   b. POST this data to The People System Supabase instance via REST API
...
3. Update popup.html and popup-bd.html: subtle branding addition:
   - Add a small "Powered by The People System" text at the bottom of the BD mode popup
   - Keep the IvyLens primary branding intact (extension still serves IvyLens too)

4. Update manifest.json:
   - Add The People System Supabase domain to host_permissions if not already present

Commit: `feat: browser extension: BD scan data writes to The People System Supabase BD intelligence tables`
```
...
2. Client invite flow from admin:
   ...
   d. The invite email should say:
      ...
      Body: "Hi [name], Tom and Lucy at The People System have set up your client portal. Click below to set your password and get started. Your first step: raise a role and run it through Friction Lens to see exactly where it will struggle before going to market."
...
4. Final README update:
   Rewrite README.md to reflect The People System:
...
5. Create their first roadmap milestone: "Onboarding Call with The People System": due [INSERT DATE]: pillar: all three: status: In Progress
```
